...
export class MCPRouter {
    ...
    findOptimalTool(capability) {
        ...
        return candidates.sort((a, b) => {
            ...
            const scoreA = metricsA.successRate * 0.5 -
                (metricsA.avgLatency / 1000) * 0.3 -
                ((a.cost || 50) / 100) * 0.2;
            const scoreB = metricsB.successRate * 0.5 -
    ...
    async route(request, securityContext) {
        ...
        console.log(`[MCPRouter.route] Received request:`, JSON.stringify({
        ...
        if (securityContext && !this.securityGateway.canExecute(tool, securityContext)) {
            throw new Error(`Permission denied: User role ${securityContext.role} cannot execute tool ${tool.name}`);
        }
        const startTime = Date.now();
        try {
            // Ensure parameters is a proper object
            const params = request.body.parameters || {};
            console.log(`[MCPRouter.route] Calling tool ${tool.name} with params:`, JSON.stringify(params, null, 2));
...
export class PredictiveEngine {
    ...
    analyzeProjectRisks(tasks, agents = []) {
        ...
        const failedCollaborations = tasks.filter((t) => t.status === "failed" && t.type === "coordination");
