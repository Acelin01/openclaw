...
export const CollaborationSkills = {
    ...
    conflictResolution: {
        ...
        detectTimelineConflict: (tasks) => {
            ...
            return tasks.filter((t) => t.status === "delayed" || (t.dueDate && new Date(t.dueDate) < new Date()));
        },
        /**
        ...
        resolveResourceConflict: (conflicts, agentPriorities) => {
            return conflicts.map((c) => ({
                ...c,
                resolution: agentPriorities[c.agentId] > 5 ? "Priority Access Granted" : "Deferred to Next Slot",
