...
export const CollaborationSkills = {
  ...
  conflictResolution: {
     ...
     */
    detectTimelineConflict: (tasks: any[]) => {
     ...
     */
    resolveResourceConflict: (conflicts: any[], agentPriorities: Record<string, number>) => {
  ...
  consensusDecision: {
    ...
    executeWeightedVote: (
      proposals: any[],
      votes: { agentId: string; weight: number; choice: string }[],
    ) => {
     ...
     */
    resolveVotingDeadlock: (votingResult: any, projectLeadId: string) => {
