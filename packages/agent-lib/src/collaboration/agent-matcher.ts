...
export class AgentMatcher {
   ...
   * 能力模型适配计算
   */
  private calculateCapabilityMatch(analysis: RequirementAnalysis, agent: Agent): number {
    let bonus = 0;
   ...
   * 权限检查
   */
  private checkPermission(agent: Agent, userRole: UserRole): boolean {
   ...
   * 技能匹配得分计算
   */
  private calculateSkillMatch(required: string[], agent: Agent, matched: string[]): number {
   ...
   * 角色/意图匹配
   */
  private isRoleMatch(agent: Agent, intent: string): boolean {
