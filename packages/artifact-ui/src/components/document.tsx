...
function PureDocumentToolResult({ type, result, isReadonly }: DocumentToolResultProps) {
  ...
  return (
    <Button
      variant="ghost"
      className="flex h-auto w-fit cursor-pointer flex-row items-start gap-3 rounded-xl border bg-background px-3 py-2 border-none"
      onClick={(event) => {
        if (isReadonly) {
        ...
        setArtifact((currentArtifact: UIArtifact) => ({
          ...
          kind: result.kind,
          content: "", // Clear content to avoid showing stale data while loading
          title: result.title,
          isVisible: true,
          status: "idle",
          boundingBox,
...
function PureDocumentToolCall({ type, args, isReadonly }: DocumentToolCallProps) {
  ...
  return (
    <Button
      variant="ghost"
      className="cursor pointer flex h-auto w-fit flex-row items-start justify-between gap-3 rounded-xl border px-3 py-2 border-none"
      onClick={(event) => {
        if (isReadonly) {
          toast.error("Viewing files in shared chats is currently not supported.");
          return;
        ...
        setArtifact((currentArtifact: UIArtifact) => {
          let docId = currentArtifact.documentId;
          if (type === "update" && "id" in args) {
            docId = args.id;
          } else if (type === "request-suggestions" && "documentId" in args) {
          ...
          return {
            ...
            isVisible: true,
            boundingBox,
            content: docId !== currentArtifact.documentId ? "" : currentArtifact.content,
          };
        });
      }}
    >
