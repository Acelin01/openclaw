...
export type ContextProps = ButtonProps & {
...
export const ContextIcon = ({ percent }: ContextIconProps) => {
  ...
  return (
    <svg
      aria-label={`${percent.toFixed(2)}% 模型上下文已使用`}
      height="28"
      role="img"
      style={{ color: "#19be6b" }}
...
export const Context = ({ className, usage, ...props }: ContextProps) => {
  ...
  // Use subscription data if available, otherwise fallback to model context usage
  const used = subscription ? subscription.used : (usage?.totalTokens ?? 0);
  const max = subscription
    ? subscription.total
    : (usage?.context?.totalMax ?? usage?.context?.combinedMax ?? usage?.context?.inputMax);
      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          className={cn(
            ...
            "cursor-pointer bg-background text-foreground h-auto p-1",
      ...
      <DropdownMenuContent align="end" className="w-fit p-3" side="top">
        <div className="min-w-[240px] space-y-2">
          <div className="flex items-start justify-between text-sm">
            <span className="font-medium">{subscription ? subscription.label : "模型上下文"}</span>
            <span>{usedPercent.toFixed(1)}%</span>
          </div>
          <div className="flex items-center justify-end text-xs text-muted-foreground">
            {hasMax
              ? `${used} / ${max} ${subscription?.unit || "Token"}`
              : `${used} ${subscription?.unit || "Token"}`}
          </div>
          ...
          {subscription ? (
            <div className="mt-2 space-y-1">
              ...
              <InfoRow label="输入" tokens={usage?.inputTokens} />
              <InfoRow label="输出" tokens={usage?.outputTokens} />
            </div>
          ) : (
