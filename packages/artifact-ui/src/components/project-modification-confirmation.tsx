      ...
      <Card className="overflow-hidden border-emerald-100 shadow-sm dark:border-emerald-900/30 dark:bg-zinc-900/50">
        <CardHeader className="bg-emerald-50/50 dark:bg-emerald-950/20 pb-3 border-b border-emerald-100/50 dark:border-emerald-900/20">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="bg-white dark:bg-zinc-800 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 text-[10px] px-1.5 py-0 h-5"
            >
              自动任务
            </Badge>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {projectData.changes.map((change, index) => (
                <div
                  key={`change-${index}-${change.field}`}
                  className="text-xs bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 rounded-lg p-2.5 space-y-1.5"
                >
                  ...
                  <div className="flex items-center gap-3 pl-2.5">
                    <span className="text-zinc-400 line-through truncate max-w-[120px]">
                      {change.oldValue || "无"}
                    </span>
                    <svg
                      width="12"
