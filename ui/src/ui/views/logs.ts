    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("logs.subtitle", "Gateway file logs (JSONL).")}</div>
        </div>
        <div class="row" style="gap: 8px;">
          <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? t("logs.action.loading", "Loading…") : t("logs.action.refresh", "Refresh")}
          </button>
          <button
            class="btn"
          ...
          >
            ${t("logs.action.export", "Export {label}", {
              label:
                exportLabel === "filtered"
                  ? t("logs.export.filtered", "filtered")
                  : t("logs.export.visible", "visible"),
            })}
          </button>
        </div>
      </div>
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="min-width: 220px;">
          <span>${t("logs.field.filter", "Filter")}</span>
          <input
            .value=${props.filterText}
            @input=${(e: Event) => props.onFilterTextChange((e.target as HTMLInputElement).value)}
            placeholder="${t("logs.field.search_placeholder", "Search logs")}"
          />
        </label>
        <label class="field checkbox">
          <span>${t("logs.field.auto_follow", "Auto-follow")}</span>
          <input
            type="checkbox"
      ...
      ${
        props.file
          ? html`<div class="muted" style="margin-top: 10px;">
              ${t("logs.file", "File: {file}", { file: props.file })}
            </div>`
          : nothing
      }
      ${
        props.truncated
          ? html`
              <div class="callout" style="margin-top: 10px">
                ${t("logs.truncated", "Log output truncated; showing latest chunk.")}
              </div>
            `
          : nothing
      }
