    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodes.subtitle", "Paired devices and live links.")}</div>
        </div>
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("nodes.action.loading", "Loading…") : t("nodes.action.refresh", "Refresh")}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.empty", "No nodes found.")}</div>
              `
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes.devices.title", "Devices")}</div>
          <div class="card-sub">${t("nodes.devices.subtitle", "Pairing requests + role tokens.")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("nodes.action.loading", "Loading…") : t("nodes.action.refresh", "Refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("nodes.devices.pending", "Pending")}</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("nodes.devices.paired", "Paired")}</div>
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">${t("nodes.devices.empty", "No paired devices.")}</div>
              `
            : nothing
        }
      </div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age = typeof req.ts === "number" ? formatAgo(req.ts) : t("nodes.label.na", "n/a");
  const role = req.role?.trim()
    ? `${t("nodes.device.role", "role")}: ${req.role}`
    : `${t("nodes.device.role", "role")}: -`;
  const repair = req.isRepair ? ` · ${t("nodes.device.repair", "repair")}` : "";
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${role} · ${t("nodes.device.requested", "requested")} ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${t("nodes.action.approve", "Approve")}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${t("nodes.action.reject", "Reject")}
          </button>
        </div>
      </div>
    </div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `${t("nodes.device.roles", "roles")}: ${formatList(device.roles)}`;
  const scopes = `${t("nodes.device.scopes", "scopes")}: ${formatList(device.scopes)}`;
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t("nodes.device.tokens_none", "Tokens: none")}</div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t("nodes.device.tokens", "Tokens")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs
    ? t("nodes.token.revoked", "revoked")
    : t("nodes.token.active", "active");
  const scopes = `${t("nodes.device.scopes", "scopes")}: ${formatList(token.scopes)}`;
