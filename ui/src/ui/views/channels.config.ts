...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout danger">${t("config.error.schema_unavailable", "Schema unavailable. Use Raw.")}</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout danger">${t("config.error.channel_schema_unavailable", "Channel config schema unavailable.")}</div>
    `;
  }
    ...
    <div style="margin-top: 16px;">
      ${
        props.configSchemaLoading
          ? html`
              <div class="muted">${t("config.schema.loading", "Loading config schema…")}</div>
            `
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? t("config.action.saving", "Saving…") : t("config.action.save", "Save")}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
