...
export function RecruitmentSidebar() {
  ...
  const tabs = RECRUITMENT_TABS.map(tab => ({
    ...tab,
    href: `/recruitment/${id}${tab.href}`,
  }));
  ...
  const categories = tabs.reduce((acc, tab: any) => {
  ...
  return (
    <aside className="w-64 bg-white border-r border-zinc-200 flex flex-col z-20">
      ...
      <div className="p-5 border-b border-zinc-100 flex items-center gap-3 bg-zinc-50/50">
        ...
        <div>
          <h2 className="text-base font-black text-zinc-900 leading-none">青椒<span className="text-[#1dbf73]">招聘</span></h2>
          <p className="text-[10px] text-zinc-500 mt-1 uppercase tracking-wider font-bold">Smart Recruitment</p>
        </div>
      </div>
