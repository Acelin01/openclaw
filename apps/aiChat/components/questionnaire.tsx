...
import { Button, Input, Label, Collapsible, CollapsibleContent, CollapsibleTrigger } from "@uxin/ui";
import { ChevronRightIcon, HelpCircle, MessageSquare, UserIcon } from "lucide-react";
            ...
            <div className="space-y-3">
              {questions.map((q) => (
                <div key={q.id} className="space-y-1">
                  <div className="text-[11px] font-medium text-zinc-400 uppercase tracking-wider">{q.label}</div>
          ...
          <div className="space-y-4">
            {questions.map((q) => (
              <div key={q.id} className="space-y-2">
                <Label htmlFor={q.id} className="text-xs font-medium text-zinc-600">
                  {q.label}
                </Label>
                <Input
                  id={q.id}
