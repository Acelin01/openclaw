...
import { fetcher, getDocumentTimestampByIndex } from '@/lib/utils';
...
export function useDocument(id: string | undefined, token?: string) {
  ...
  const restoreMutation = useMutation({
    ...
    onMutate: async ({ documentId, timestamp }) => {
      ...
      const previousDocuments = queryClient.getQueryData<Document[]>(['document', documentId]);

      queryClient.setQueryData<Document[]>(['document', documentId], (old) => {
        if (!old) return [];
        // Optimistic update logic from version-footer.tsx
        // Keep documents that are NOT after the timestamp (i.e. keep older or equal versions)
